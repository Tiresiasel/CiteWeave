...
class EnhancedMultiAgentSystem:
    ...
    def log_agent_trace(self, agent_name, input_data, output_data, extra=None):
        ...
        # If no current query ID, create a new one
        if not self.current_query_id and self.current_query:
            try:
                self.current_query_id = self.trace_storage.start_query_trace(
                    query_text=self.current_query,
                    thread_id=self.current_thread_id,
                    user_id=self.current_user_id,
                    query_type=getattr(self, 'query_type', 'unknown'),
                    language=getattr(self, 'user_language', 'en')
                )
                logger.info(f"Started new query trace: {self.current_query_id}")
            except Exception as e:
                logger.error(f"Failed to start query trace: {e}")
                return
        
        # Calculate execution time and memory usage
        execution_time = extra.get('execution_time', 0.0) if extra else 0.0
        memory_usage = extra.get('memory_usage', 0) if extra else 0
        error_message = extra.get('error_message') if extra else None
        
        # Record step to persistent storage
        if self.current_query_id:
            try:
                step_number = len(self.agent_trace_log) + 1
                self.trace_storage.add_query_step(
                ...
                logger.error(f"Failed to save step to storage: {e}")
    ...
    def agent_trace_decorator(agent_func):
        async def wrapper(self, state, *args, **kwargs):
            ...
            try:
                ...
                self.log_agent_trace(
                    agent_func.__name__, 
                    input_data=None, 
                    output_data=result.__dict__ if hasattr(result, "__dict__") else result,
                    extra={
                        "execution_time": execution_time,
                        "memory_usage": memory_usage
                    }
        ...
        try:
            ...
            if self.current_query_id:
                try:
                    ...
                    data_sources = []
                    if hasattr(result, 'route_results') and result.route_results:
                        data_sources = list(result.route_results.keys())
                    ...
                    logger.info(f"Completed query trace: {self.current_query_id}")
